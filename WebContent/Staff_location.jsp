...
<body>
	...
	<div class="content p-3">
		...
		<div class="card mb-2 w-100">
			<div class="card-header fw-bold">Add Location</div>
			<div class="card-body">
				<div class="container">
					<div class="row g-5">
						<form action="Add_Location_Servlet" method="post" class="needs-validation" novalidate>
							...
							<div class="row g-3">
								<div class="col-6">
									...
									<input name="location_idnum" type="text" class="form-control" required>
									<div class="invalid-feedback"> ID number is required. </div>
								</div>
								<div class="col-6">
									<label class="form-label">Block Name</label>
									<input style="text-transform: capitalize;" name="location_block" type="text" class="form-control" required>
					...
					<tbody>
						<c:forEach items="${locationlist}" var="loclist">
							<tr>
								...
								<td>
									<fmt:formatDate pattern = "dd-MM-yyyy" value = "${loclist.location_maint_date}"/><br>
									<label class="text-black-50"><c:out value="${loclist.location_date_different}"></c:out></label>
								</td>
								<td>
									<c:choose>
										<c:when test="${loclist.location_maint_status == 'Complete'}">
											<div class="alert alert-success text-center w-75 p-2" role="alert">
												<c:out value="${loclist.location_maint_status}"></c:out>
											</div>
										</c:when>
										<c:when test="${loclist.location_maint_status == 'Not Complete'}">
											<div class="alert alert-danger text-center w-75 p-2" role="alert">
												<c:out value="${loclist.location_maint_status}"></c:out>
											</div>
										</c:when>
										<c:when test="${loclist.location_maint_status == 'In Progress'}">
											<div class="alert alert-warning text-center w-75 p-2" role="alert">
												<c:out value="${loclist.location_maint_status}"></c:out>
											</div>
										</c:when>
									</c:choose>
								...
								<td>
									<button onclick="window.location.href='Redirect_Servlet?action=location_update&id=<c:out value="${loclist.location_idnum}"></c:out>'" class="btn btn-sm m-1 btn-secondary" style="border: none;"><img src="assets/icons/edit_white.svg" class="py-1"></button>
									<button onclick="window.location.href='Redirect_Servlet?action=location_delete&id=<c:out value="${loclist.location_idnum}"></c:out>'" class="btn btn-sm m-1 btn-danger" style="border: none;"><img src="assets/icons/delete_white.svg" class="py-1"></button>
								</td>
							</tr>
						</c:forEach>
					</tbody>
		...
		<c:if test="${session_noti != null}">
			<c:if test="${session_noti == true}">
				<div class="position-fixed bottom-0 end-0 px-4 py-2" style="z-index: 11">
					<div class="alert alert-danger alert-dismissible fade show" role="alert">
				  		<strong>Attention!</strong> New complaint had been issued
				  		<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
					</div>
				</div>
			</c:if>
			<c:set var="session_noti" value="${null}"></c:set>
		</c:if>
	</div>
